...
class SimulationGraphicConfig:
    ...
    VEHICLE_WIDTH = 80
    VEHICLE_HEIGHT = 60
    VERTICAL_ROAD_START_Y = SCREEN_HEIGHT
    VERTICAL_ROAD_LEFT_LANE_START_X = (SCREEN_WIDTH // 2) - (ROAD_WIDTH // 4) - (VEHICLE_HEIGHT // 2)
    VERTICAL_ROAD_RIGHT_LANE_START_X = (SCREEN_WIDTH // 2) + (ROAD_WIDTH // 4) - (VEHICLE_HEIGHT // 2)
    HORIZONTAL_ROAD_START_X = SCREEN_WIDTH
    HORIZONTAL_ROAD_LEFT_LANE_START_Y = (SCREEN_HEIGHT // 2) + (ROAD_WIDTH // 4) - (VEHICLE_HEIGHT // 2)
    HORIZONTAL_ROAD_RIGHT_LANE_START_Y = (SCREEN_HEIGHT // 2) - (ROAD_WIDTH // 4) - (VEHICLE_HEIGHT // 2)
    LANE_STARTING_POSITIONS = {
        "vertical_road_left_lane": [VERTICAL_ROAD_LEFT_LANE_START_X, VERTICAL_ROAD_START_Y],
        "vertical_road_right_lane": [VERTICAL_ROAD_RIGHT_LANE_START_X, VERTICAL_ROAD_START_Y],
    ...
    STOP_LINE_POSITIONS = {
        "vertical_road": SCREEN_HEIGHT // 2 + (ROAD_WIDTH // 2),
        "horizontal_road": SCREEN_WIDTH // 2 + (ROAD_WIDTH // 2)
    }
    VEHICLE_SPAWN_GAP_METERS = 0  # Minimum gap between spawned vehicles in meters
    VEHICLE_MIN_GAP_METERS = 0    # Minimum gap between vehicles while moving in meters
